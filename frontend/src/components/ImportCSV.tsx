...
// ── CSV detection (mirrors server-side logic) ─────────────────────────────
...
// ── Component ─────────────────────────────────────────────────────────────
const ImportCSV: React.FC<ImportCSVProps> = ({ refresh }) => {
    ...
    <div className="border border-green-900/60 p-4 sm:p-5 font-jersey15 bg-black text-green-600">
      ...
      <div
        ...
        className={`border border-dashed transition cursor-pointer p-5 text-center mb-4 outline-none
          ${dragActive ? "border-green-400 bg-green-950/30 scale-[1.01]" :
      ...
      >
        ...
        {file ? (
          ...
          <div className="flex flex-col items-center gap-1">
            <Icon
              icon={dragActive ? "pixelarticons:arrow-down" : "pixelarticons:upload"}
              width={28}
              className={dragActive ? "text-green-400" : "text-green-800"}
            />
            <p className={`text-sm ${dragActive ? "text-green-400" : "text-green-700"}`}>
              {dragActive ? "Release to upload" : "Drop CSV here or click to select"}
            </p>
      ...
      {sizeWarning && (
        <div className="border border-yellow-700/40 bg-yellow-950/20 p-2.5 mb-3 text-xs text-yellow-500">
          ⚠ File is larger than {MAX_SIZE_MB} MB — it will still be processed but may take longer.
        </div>
      )}
      ...
      {file && (
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <button onClick={() => setManualOverride(null)}        className={modeCls(!manualOverride)}>Auto-detect</button>
            <button onClick={() => setManualOverride("balance")}   className={modeCls(manualOverride === "balance")}>Balance History</button>
            <button onClick={() => setManualOverride("order")}     className={modeCls(manualOverride === "order")}>Order History</button>
            ...
            {detectedType === "unknown" && !manualOverride && (
              <span className="text-xs px-2 py-0.5 border border-yellow-700/60 text-yellow-500">
                ⚠ Unknown format
              </span>
            )}
          </div>
      ...
      {activeType === "order" && (
        <div className="border border-yellow-700/40 bg-yellow-950/20 p-3 mb-4 text-xs text-yellow-500 leading-relaxed">
          ...
          correctly if exported from your{" "}
          <span className="text-yellow-400">very first trade</span>. A partial export will
          produce wrong sides near the top of the file.{" "}
      ...
      <button
        ...
        disabled={isDisabled}
        className={`w-full p-3 border text-base transition ${
          isDisabled
